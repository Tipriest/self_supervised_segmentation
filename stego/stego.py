...
class Stego(pl.LightningModule):
    ...
    def reset_clusters(self, n_classes, extra_clusters):
        """
        Resets STEGO's cluster and linear probes, possibly with a different
        number of classes and extra clusters for the cluster probe.
        """
    ...
    def postprocess_cluster(
        self, code, img, use_crf=True, image_clustering=False
    ):
        """
        ...
        if image_clustering:
            ...
            normed_centers = F.normalize(
                self.kmeans.final_cluster_centers, dim=1
            )
            normed_code = F.normalize(code, dim=1).permute(0, 2, 3, 1)
            inner_products = torch.einsum(
                "bhwc,nc->bnhw", normed_code, normed_centers
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.cd_hist = torch.add(
            self.cd_hist,
            torch.histc(pos_intra_cd.cpu(), bins=40, min=-1, max=1),
        )
        self.cd_hist = torch.add(
            self.cd_hist,
            torch.histc(pos_inter_cd.cpu(), bins=40, min=-1, max=1),
        )
        self.cd_hist = torch.add(
            self.cd_hist,
            torch.histc(neg_inter_cd.cpu(), bins=40, min=-1, max=1),
        )
        ...
        linear_logits = linear_logits.permute(0, 2, 3, 1).reshape(
            -1, self.n_classes
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            self.log(
                "val/linear/Accuracy", linear_metrics["test/linear/Accuracy"]
            )
            self.log("val/cluster/mIoU", cluster_metrics["test/cluster/mIoU"])
            self.log(
                "val/cluster/Accuracy", cluster_metrics["test/cluster/Accuracy"]
            )
            self.validation_step_outputs.append(
                {
                    "img": img[: self.cfg.val_n_imgs].detach().cpu(),
                    "linear_preds": linear_preds[: self.cfg.val_n_imgs]
                    .detach()
                    .cpu(),
                    "cluster_preds": cluster_preds[: self.cfg.val_n_imgs]
                    .detach()
                    .cpu(),
