...
def get_backbone(cfg):
    ...
    After implementing the Backbone class for your backbone,
    add it to be returned from this function with a desired named.
    ...
    if cfg.backbone == "dino":
        ...
        raise ValueError(f"Backbone {cfg.backbone} unavailable")
...
class Dinov2ViT(Backbone):
    def __init__(self, cfg):
        ...
        if self.backbone_type == "vit_small":
            ...
            raise ValueError(f"Model type {cfg.backbone_type} unavailable")

        for p in self.model.parameters():
            p.requires_grad = False
        self.model.eval().cuda()
        self.dropout = torch.nn.Dropout2d(
            p=np.clip(self.cfg.dropout_p, 0.0, 1.0)
        )
    ...
    def get_backbone_name(self):
        return (
            "DINOv2-"
            + Backbone.vit_name_long_to_short[self.backbone_type]
            + "-"
            + str(self.patch_size)
        )
...
class DinoViT(Backbone):
    def __init__(self, cfg):
        ...
        self.backbone_type = self.cfg.backbone_type
        self.model = vits.__dict__[self.backbone_type](
        ...
        if self.backbone_type == "vit_small" and self.patch_size == 16:
            ...
            raise ValueError(
                f"Model type {self.backbone_type} unavailable with patch size {self.patch_size}"
            )
